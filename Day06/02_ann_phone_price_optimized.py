...
def train(train_dataset, input_dim, class_num):
    ...
    # 模型保存
    torch.save(model.state_dict(), os.path.join('model', 'phone-price-model2.pth'))
...
def test(valid_dataset, input_dim, class_num):
    ...
    # load:加载用来保存模型参数的文件
    model.load_state_dict(torch.load(os.path.join('model', 'phone-price-model2.pth')))
    # 构建加载器
